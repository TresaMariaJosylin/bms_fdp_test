...
The six tests verify valid record insertion, movie retrieval, average rating, highest-rated movie, genre summary, invalid rating rejection, favorites, update behavior, inactive-status deletion, restore behavior, and login credentials. Tests use a temporary database and do not modify the project collection.
...
| --- | --- | --- | --- |
| Database creation | Start the Streamlit app | `movies.db` and the `movies` table are created automatically | Screenshot of SQLite database/table |
| Login | Enter `admin` and `movie123` | Dashboard opens | Screenshot of dashboard after sign in |
...
| Deactivate record | Select a movie, confirm deactivation, and deactivate | Record remains visible with `Inactive` status and is excluded from active statistics | Screenshot of inactive record |
| Restore record | Select an inactive movie and choose `Restore Movie` | Record status changes back to `Active` | Screenshot of restored record |
...
| Favorite movie | Mark a movie as a favorite | Favorite flag, dashboard count, and favorites filter update | Screenshot of favorite collection |
